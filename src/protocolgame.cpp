...
void ProtocolGame::onRecvFirstMessage(NetworkMessage& msg)
{
  ...
  // this will be called after you start connecting to character already
...
void ProtocolGame::MoveUpCreature(NetworkMessage& msg, const Creature* creature, const Position& newPos, const Position& oldPos)
{
	...
	if (newPos.z == 7) {
		...
		for (int i = 5; i >= 0; --i) {
			GetFloorDescription(msg, oldPos.x - Map::maxClientViewportX, oldPos.y - Map::maxClientViewportY, i, (Map::maxClientViewportX+1)*2, (Map::maxClientViewportY+1)*2, 8 - i, skip);
		}
...
void ProtocolGame::MoveDownCreature(NetworkMessage& msg, const Creature* creature, const Position& newPos, const Position& oldPos)
{
	...
	if (newPos.z == 8) {
		...
		for (int i = 0; i < 3; ++i) {
			GetFloorDescription(msg, oldPos.x - Map::maxClientViewportX, oldPos.y - Map::maxClientViewportY, newPos.z + i, (Map::maxClientViewportX + 1) * 2, (Map::maxClientViewportY + 1) * 2, -1 - i, skip);
		}
		if (skip >= 0) {
			msg.addByte(skip);
			msg.addByte(0xFF);
...
void ProtocolGame::AddShopItem(NetworkMessage& msg, const ShopInfo& item)
{
	...
	msg.add<uint32_t>(std::max<int64_t>(std::min<int64_t>(item.buyPrice, std::numeric_limits<uint32_t>::max()), 0));
	msg.add<uint32_t>(std::max<int64_t>(std::min<int64_t>(item.sellPrice, std::numeric_limits<uint32_t>::max()), 0));
	msg.add<uint32_t>(item.funcShop);
}
