...
function Player:onGainExperience(source, exp, rawExp)
	if hasEventCallback(EVENT_CALLBACK_ONGAINEXPERIENCE) then
		return EventCallback(EVENT_CALLBACK_ONGAINEXPERIENCE, self, source, exp, rawExp)
...
function Player:onGainSkillTries(skill, tries)
	if(hasEventCallback(EVENT_CALLBACK_ONGAINSKILLTRIES)) then
		return EventCallback(EVENT_CALLBACK_ONGAINSKILLTRIES, self, skill, tries)
	end
...
function Player:onWrapItem(item)
	if(hasEventCallback(EVENT_CALLBACK_ONWRAPITEM)) then
		return EventCallback(EVENT_CALLBACK_ONWRAPITEM, self, item)
	end
